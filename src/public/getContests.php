...
$counts = array('new' => 0, 'existing' => 0, 'updated'=> 0, 'api' => 0, 'html' => 0, 'newcontests' => array());
...
$counts['contests'][$contests[1][$i]] = $contests[2][$i];
print_r($counts['contests']);
...
# Set url
curl_setopt($chwh, CURLOPT_URL,$webhookURL); 


# Return the transfer as a string
curl_setopt($chwh, CURLOPT_CUSTOMREQUEST, "POST");                                                                     
curl_setopt($chwh, CURLOPT_POSTFIELDS, $payload);                                                                  
curl_setopt($chwh, CURLOPT_RETURNTRANSFER, true);                                                                      
curl_setopt($chwh, CURLOPT_HTTPHEADER, array(                                                                          
'Content-Type: application/json',                                                                                
'Content-Length: ' . strlen($payload))                                                                       
);
#contains the output string
$slackoutput = curl_exec($chwh);
  print_r($slackoutput); 
...
$chwh = curl_init();
  ...
  # Return the transfer as a string
  curl_setopt($chwh, CURLOPT_CUSTOMREQUEST, "POST");                                                                      curl_setopt($chwh, CURLOPT_POSTFIELDS, $payload);
  curl_setopt($chwh, CURLOPT_RETURNTRANSFER, true);                                                                       curl_setopt($chwh, CURLOPT_HTTPHEADER, array(                                                                         
    'Content-Type: application/json',
    'Content-Length: ' . strlen($payload))    
  );
  #contains the output string
